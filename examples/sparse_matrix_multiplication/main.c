...
int main(){
	...
	copyBufferSlow(ctx, cpu_stage, buf_inputs, 0, 0, inSz);

	/* update indices */
	p = mapBuffer(ctx, cpu_stage);
	memcpy(p, h_update, upSz);
	unmapBuffer(ctx, cpu_stage);
	copyBufferSlow(ctx, cpu_stage, buf_update, 0, 0, upSz);

	/* start_positions */
	p = mapBuffer(ctx, cpu_stage);
	memcpy(p, h_start, stSz);
	unmapBuffer(ctx, cpu_stage);
	copyBufferSlow(ctx, cpu_stage, buf_start, 0, 0, stSz);

	/* to_indices */
	p = mapBuffer(ctx, cpu_stage);
	memcpy(p, h_toIdx, idxSz);
	unmapBuffer(ctx, cpu_stage);
	copyBufferSlow(ctx, cpu_stage, buf_toIdx, 0, 0, idxSz);

	/* weights */
	p = mapBuffer(ctx, cpu_stage);
	memcpy(p, h_weights, wSz);
	unmapBuffer(ctx, cpu_stage);
	copyBufferSlow(ctx, cpu_stage, buf_weights, 0, 0, wSz);
	...
	ip[2] = 1;
	unmapBuffer(ctx, cpu_stage);
	copyBufferSlow(ctx, cpu_stage, buf_indirect, 0, 0, buf_indirect.size);
	...
	runProgram(ctx, cmd, prog, buf_indirect);
	submitCommand(ctx, cmd);

	/* ---- read back ------------------------------------------------------- */
	copyBufferSlow(ctx, buf_outputs, cpu_stage, 0, 0, outSz);
