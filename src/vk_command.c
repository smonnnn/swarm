...
void copyBufferSlow(VKCTX ctx, VKBUFFER from, VKBUFFER to, uint32_t from_offset, uint32_t to_offset, uint32_t size){
    ...
    copyBuffer(ctx, cmd, from, to, from_offset, to_offset, size);
...
void runProgram(VKCTX ctx, VkCommandBuffer cmd, VKPROGRAM program, VKBUFFER indirect){
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, program.pipeline_layout, 0, 1, &program.descriptor_set, 0, NULL);
    
    vkCmdDispatchIndirect(cmd, indirect.buffer, 0);
    VkBufferMemoryBarrier barriers[program.buffer_count]; //Should be the max amount of barriers possible for the given buffers.
    uint32_t barrierCount = 0;

    for (uint32_t j = 0; j < program.buffer_count; ++j) {
        BindingLimitations lim = program.binding_read_write_limitations[j];
        if (lim == WRITE_ONLY || lim == READ_AND_WRITE) {
            barriers[barrierCount++] = (VkBufferMemoryBarrier){
                ...
                .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                .buffer              = program.buffers[j].buffer,
                .offset              = 0,
                .size                = VK_WHOLE_SIZE
            };
        }
